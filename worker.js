export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/health") return json({ok:true,name:"Mudabbir",version:"2-cloudflare"});
    ...
    if (url.pathname === "/api/projects" && request.method === "POST") {
      ...
      if (!goal) return json({ok:false,error:"اكتب هدف المشروع"},400);
      ...
      const names = ["فهم الهدف","بناء خطة التنفيذ","البحث والتحليل","إنتاج النتيجة","المراجعة والتسليم"];
      for (const name of names)
        await env.DB.prepare("INSERT INTO tasks(project_id,title) VALUES(?,?)").bind(projectId,name).run();
